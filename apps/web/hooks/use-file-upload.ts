...
export function useFileUpload({
  ...
  const completeUpload = useMutation(api.private.attachments.completeUpload)
  ...
  const uploadFile = useCallback(
    async (attachment: PendingAttachment): Promise<PendingAttachment> => {
      if (!conversationId) {
    ...
    [conversationId, generateUploadUrl, completeUpload, onError],
  )
